    ...
    try:
        if render_type in ['FML', 'LD', 'HD']:
            ...
            if wizard_export.trigger_sanity_hook('lighting', exported_string_asset):
                ...
                render_directory = wizard_render.setup_render_directory(
                    'lighting', export_name)
                wizard_export.trigger_after_export_hook(
                    'lighting', render_directory, exported_string_asset)
                if not farm:
                    batch()
                else:
                    deadline()
