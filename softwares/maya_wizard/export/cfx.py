...
def main_abc(nspace_list, frange, comment=''):
    ...
    if references:
        for reference in references:
            ...
            if reference['namespace'] in nspace_list:
                at_least_one = True
                export_cfx_abc(reference, frange,
...
def invoke_settings_widget():
    ...
    export_settings_widget_win = export_settings_widget.export_settings_widget(
        'cfx', parent=wizard_tools.maya_main_window())
    ...
    if is_referenced(grooming_nspace):
        ...
        if export_GRP_list:
            ...
            additionnal_objects = wizard_export.trigger_before_export_hook(
                'cfx', exported_string_asset)
            export_GRP_list += additionnal_objects
            export_name = buid_export_name(asset_name, count)
            wizard_export.export('cfx', export_name, exported_string_asset,
                                 export_GRP_list, frange, percent_factor=percent_factor, comment=comment)
        else:
            logger.warning(
                "No objects to export in '{}:yeti_nodes_set'".format(grooming_nspace))
...
def export_cfx_abc(reference, frange, percent_factor, comment=''):
    ...
    if is_referenced(nspace):
        ...
        if export_GRP_list:
            ...
            logger.warning(
                "No objects to export in '{}:render_set'".format(nspace))
