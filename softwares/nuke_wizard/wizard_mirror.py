...
class mirror_files_to_local():
    ...
    def mirror(self):
        ...
        if local_path is None or local_path == '':
            logger.warning(
                "Local path not setted, keeping project files. It is recommended to set a local path in wizard.")
            return local_files_list
        for file in self.files_list:
            local_file = replace_project_path_with_local_path(
                file, project_path, local_path)
            if not os.path.isdir(os.path.dirname(local_file)):
                os.makedirs(os.path.dirname(local_file))
            if not os.path.isfile(local_file):
                if nuke.GUI:
                    self.mirror_loading_widget.set_info(
                        f"Copying {os.path.basename(file)}...")
                logger.info(f"Copying {os.path.basename(file)}...")
                shutil.copyfile(file, local_file)
            else:
                if nuke.GUI:
                    self.mirror_loading_widget.set_info(
                        f"{os.path.basename(file)} already exists, skipping")
                logger.info(
                    f"{os.path.basename(file)} already exists, skipping")
