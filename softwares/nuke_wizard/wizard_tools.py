...
def exr_list_to_paths_list(exr_list):
    ...
    for file in exr_list:
        digits_match = re.search(r'(_|\.)([0-9]+)(.exr)', file)
        if digits_match is not None:
            digits = int(digits_match.group(2))
            num_digits = len(digits_match.group(2))
            hashtags = '#' * num_digits
            file_name_with_hashtags = re.sub(
            r'(_|\.)[0-9]+(.exr)', rf'\1{hashtags}\2', file)
        else:
            digits = ''
            file_name_with_hashtags = file
    ...
    for file_name_with_hashtags in paths_dic.keys():
        ...
        frange = [paths_dic[file_name_with_hashtags]
                  [0], paths_dic[file_name_with_hashtags][-1]]
...
def switch_selection_to_deepRead():
    ...
    for node in selection:
        if node.Class() == 'Read':
            new_node = nuke.nodes.DeepRead(
                file='', name='temp', xpos=0, ypos=-500)
...
def replace_node(node, new_node, all_nodes):
    ...
    set_read_data(new_node, node['file'].value(), [
                  node['first'].value(), node['last'].value()])
