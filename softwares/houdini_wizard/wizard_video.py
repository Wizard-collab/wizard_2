...
def flipbook(directory, frange, camera, focal_lengths_dic, comment=''):
    ...
    opengl_node.parm('postrender').set(publish_video_script(
        directory, frange, focal_lengths_dic, comment=comment))
...
def create_rop_network():
    obj_node = hou.node("/obj")
    opengl_node = wizard_tools.look_for_node(
        'wizard_opengl_flipbook', parent=obj_node)
