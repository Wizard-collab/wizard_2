...
def add_domain(name):
    domain_id = db_utils.create_row(level,
                        'domains', 
...
def get_domains():
    domain_rows = db_utils.get_rows(level, 'domains')
    return domain_rows

def get_domain_data(domain_id, column='*'):
    domain_rows = db_utils.get_row_by_column_data(level,
                                                    'domains',
                                                    ('id', domain_id),
                                                    column)
...
def get_domain_childs(domain_id, column='*'):
    categories_rows = db_utils.get_row_by_column_data(level,
                                                    'categories',
                                                    ('domain_id', domain_id),
                                                    column)
...
def remove_domain(domain_id):
    ...
    if site.is_admin():
        ...
        success = db_utils.delete_row(level, 'domains', domain_id)
        if success:
            logging.info(f"Domain removed from project")
    return success

def get_all_categories(column='*'):
    categories_rows = db_utils.get_rows(level,
                                            'categories',
                                            column)
    return categories_rows

def add_category(name, domain_id):
    if not (db_utils.check_existence_by_multiple_data(level, 
                                    'categories',
                                    ('name', 'domain_id'),
                                    (name, domain_id))):
        category_id = db_utils.create_row(level,
                            'categories',
...
def remove_category(category_id):
    ...
    if site.is_admin():
        ...
        success = db_utils.delete_row(level, 'categories', category_id)
        if success:
            logging.info(f"Category removed from project")
    return success

def get_category_childs(category_id, column="*"):
    assets_rows = db_utils.get_row_by_column_data(level,
                                                    'assets',
...
def get_category_data(category_id, column='*'):
    category_rows = db_utils.get_row_by_column_data(level,
                                                    'categories',
                                                    ('id', category_id),
                                                    column)
...
def add_asset(name, category_id):
    if not (db_utils.check_existence_by_multiple_data(level, 
                                    ...
                                    ('name', 'category_id'),
                                    (name, category_id))):
        asset_id = db_utils.create_row(level,
                            'assets', 
...
def get_all_assets(column='*'):
    assets_rows = db_utils.get_rows(level,
                                            'assets',
                                            column)
    return assets_rows
...
def search_asset(name, category_id=None, column='*'):
    if category_id:
        asset_rows = db_utils.get_row_by_column_part_data_and_data(level,
                                                        'assets',
                                                        ('name', name),
                                                        ('category_id', category_id),
                                                        column)
    else:
        asset_rows = db_utils.get_row_by_column_part_data(level,
                                                            'assets',
                                                            ('name', name),
                                                            column)
    return asset_rows
...
def remove_asset(asset_id):
    ...
    if site.is_admin():
        ...
        success = db_utils.delete_row(level, 'assets', asset_id)
        if success:
            logging.info(f"Asset removed from project")
    return success

def get_asset_childs(asset_id, column='*'):
    stages_rows = db_utils.get_row_by_column_data(level,
                                                        'stages',
                                                        ('asset_id', asset_id),
                                                        column)
    return stages_rows
...
def add_stage(name, asset_id):
    if not (db_utils.check_existence_by_multiple_data(level, 
                                    ...
                                    ('name', 'asset_id'),
                                    (name, asset_id))):
        stage_id = db_utils.create_row(level,
                            'stages', 
                            ('name',
                                'creation_time',
                                'creation_user',
...
def get_all_stages(column='*'):
    stages_rows = db_utils.get_rows(level,
                                            'stages',
                                            column)
    return stages_rows
...
def remove_stage(stage_id):
    ...
    if site.is_admin():
        ...
        success = db_utils.delete_row(level, 'stages', stage_id)
        if success:
...
def get_stage_childs(stage_id, column='*'):
    variants_rows = db_utils.get_row_by_column_data(level,
                                                        'variants',
                                                        ('stage_id', stage_id),
                                                        column)
...
def add_variant(name, stage_id, comment):
    if not (db_utils.check_existence_by_multiple_data(level, 
                                    ...
                                    ('name', 'stage_id'),
                                    (name, stage_id))):
        variant_id = db_utils.create_row(level,
                            'variants', 
                            ('name',
                                'creation_time',
                                'creation_user',
...
def remove_variant(variant_id):
    ...
    if site.is_admin():
        ...
        success = db_utils.delete_row(level, 'variants', variant_id)
        if success:
...
def get_variant_work_envs_childs(variant_id, column='*'):
    work_envs_rows = db_utils.get_row_by_column_data(level, 
                                                        'work_envs', 
...
def get_variant_export_childs(variant_id, column='*'):
    exports_rows = db_utils.get_row_by_column_data(level, 
                                                        'exports', 
                                                        ('variant_id', variant_id), 
                                                        column)
    return exports_rows
...
def get_export_by_name(name, variant_id):
    export_row = db_utils.get_row_by_multiple_data(level, 
                                                        'exports', 
                                                        ('name', 'variant_id'), 
                                                        (name, variant_id))
...
def get_export_data(export_id, column='*'):
    export_rows = db_utils.get_row_by_column_data(level, 
                                                        'exports', 
                                                        ('id', export_id), 
                                                        column)
...
def get_export_childs(export_id, column='*'):
    exports_versions_rows = db_utils.get_row_by_column_data(level, 
                                                        'export_versions', 
                                                        ('export_id', export_id), 
                                                        column)
...
def is_export(name, variant_id):
    return db_utils.check_existence_by_multiple_data(level, 
                                    'exports',
                                    ('name', 'variant_id'),
                                    (name, variant_id))
...
def add_export(name, variant_id):
    if not (db_utils.check_existence_by_multiple_data(level, 
                                    ...
                                    ('name', 'variant_id'),
                                    (name, variant_id))):
        export_id = db_utils.create_row(level,
                            'exports', 
                            ('name',
                                'creation_time',
                                'creation_user',
...
def remove_export(export_id):
    ...
    if site.is_admin():
        ...
        success = db_utils.delete_row(level, 'exports', export_id)
        if success:
            logging.info("Export removed from project")
    return success

def get_export_versions(export_id, column='*'):
    export_versions_rows = db_utils.get_row_by_column_data(level,
                                                        'export_versions',
                                                        ('export_id', export_id),
                                                        column)
...
def get_last_export_version(export_id, column='*'):
    versions_rows = db_utils.get_last_row_by_column_data(level,
                                                        'export_versions',
                                                        ('export_id', export_id),
                                                        column)
...
def add_export_version(name, files, export_id, work_version_id=None, comment=''):
    if not (db_utils.check_existence_by_multiple_data(level, 
                                    ...
                                    ('name', 'export_id'),
                                    (name, export_id))):
        export_version_id = db_utils.create_row(level,
                            'export_versions', 
                            ('name',
                                'creation_time',
                                'creation_user',
...
def get_export_version_destinations(export_version_id, column='*'):
    references_rows = db_utils.get_row_by_column_data(level,
                                                        'references_data',
...
def remove_export_version(export_version_id):
    ...
    if site.is_admin():
        ...
        success = db_utils.delete_row(level, 'export_versions', export_version_id)
        if success:
            logging.info("Export version removed from project")
    return success

def get_export_version_tickets(export_version_id, column='*'):
    tickets_rows = db_utils.get_row_by_column_data(level,
                                                        'tickets',
                                                        ('export_version_id', export_version_id),
                                                        column)
    return tickets_rows
...
def add_work_env(name, software_id, variant_id):
    if not (db_utils.check_existence_by_multiple_data(level, 
                                    ...
                                    ('name', 'variant_id'),
                                    (name, variant_id))):
        work_env_id = db_utils.create_row(level,
                            'work_envs', 
                            ('name',
                                'creation_time',
                                'creation_user',
...
def create_reference(work_env_id, export_version_id, namespace):
    ...
    if not (db_utils.check_existence_by_multiple_data(level, 
                                    ...
                                    (namespace, work_env_id))):
        reference_id = db_utils.create_row(level,
                                'references_data', 
                                ('creation_time',
                                    'creation_user',
                                    'namespace',
...
def remove_reference(reference_id):
    success = db_utils.delete_row(level, 'references_data', reference_id)
    if success:
...
def remove_work_env(work_env_id):
    ...
    if site.is_admin():
        ...
        for reference_id in get_references(work_env_id, 'id'):
            remove_reference(reference_id)
        success = db_utils.delete_row(level, 'work_envs', work_env_id)
        if success:
            logging.info("Work env removed from project")
    return success

def get_work_versions(work_env_id, column='*'):
    versions_rows = db_utils.get_row_by_column_data(level,
                                                        'versions',
                                                        ('work_env_id', work_env_id),
                                                        column)
    return versions_rows
...
def get_work_env_data(work_env_id, column='*'):
    work_env_rows = db_utils.get_row_by_column_data(level,
                                                        'work_envs',
                                                        ('id', work_env_id),
                                                        column)
...
def add_version(name, file_path, work_env_id, comment='', screenshot_path=None):
    version_id = db_utils.create_row(level,
                        'versions', 
                        ('name',
                            'creation_time',
                            'creation_user',
...
def remove_version(version_id):
    success = None
    if site.is_admin():
        success = db_utils.delete_row(level, 'versions', version_id)
        if success :
...
def add_software(name, extension, file_command, no_file_command):
    if name in softwares_vars._softwares_list_:
        if name not in get_softwares_names_list():
            software_id = db_utils.create_row(level,
                            'softwares', 
                            ('name', 
                                'extension',
                                'path',
...
def get_softwares_names_list():
    softwares_rows = db_utils.get_rows(level, 'softwares', 'name')
...
def get_software_data(software_id, column='*'):
    softwares_rows = db_utils.get_row_by_column_data(level,
                                                        'softwares',
                                                        ('id', software_id),
                                                        column)
...
def create_settings_row(frame_rate, image_format):
    if len(db_utils.get_rows(level, 'settings', 'id'))==0:
        if db_utils.create_row(level,
                                            'settings',
                                            ('frame_rate',
                                                'image_format',
...
def get_frame_rate():
    frame_rate_list = db_utils.get_row_by_column_data(level,
                                                        'settings',
                                                        ('id', 1),
                                                        'frame_rate')
...
def get_image_format():
    image_format_list = db_utils.get_row_by_column_data(level,
                                                        'settings',
                                                        ('id', 1),
                                                        'image_format')
...
def get_users_ids_list():
    users_ids_list = db_utils.get_row_by_column_data(level,
                                                        'settings',
                                                        ('id', 1),
                                                        'users_ids')
    ...
    if is_user:
        if copied_files is not None:
            if export_id:
                ...
                if variant_id:
                    ticket_id = db_utils.create_row(level,
                                                        'tickets',
                                                        ('creation_user',
                                                            'creation_time',
                                                            'title',
...
def remove_ticket(ticket_id):
    success = None
    if site.is_admin():
        success = db_utils.delete_row(level, 'tickets', ticket_id)
        if success :
...
def add_event(event_type, message, data):
    event_id = db_utils.create_row(level,
                                            'events',
                                            ('creation_user',
                                                'creation_time',
                                                'type',
...
def get_event_data(event_id, column='*'):
    events_rows = db_utils.get_row_by_column_data(level,
                                                        'events',
                                                        ('id', event_id),
                                                        column)
...
def add_shelf_script(name,
    ...
    if not db_utils.check_existence(level, 'shelf_scripts', 'name', name):
        if os.path.isfile(icon):
            icon = ressources._default_script_shelf_icon_

        shelf_script_id = db_utils.create_row(level,
                                                'shelf_scripts',
                                                ('creation_user',
                                                    'creation_time',
                                                    'name',
...
def get_shelf_script_data(name, column='*'):
    shelf_scripts_rows = db_utils.get_row_by_column_data(level,
                                                        'shelf_scripts',
                                                        ('name', name),
                                                        column)
