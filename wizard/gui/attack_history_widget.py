    ...
    def refresh(self):
        ...
        for attack_row in attack_rows[-event_number:]:
            if attack_row['id'] not in self.attack_ids.keys():
                ...
                self.events_content_layout.addWidget(self.attack_ids[attack_row['id']]['widget'])
...
class attack_event_widget(QtWidgets.QFrame):
    ...
    def fill_ui(self):
        ...
        self.attack_info_label.setText(f"( {game_vars.artefacts_dic[self.attack_row['artefact']]['description']} )")
        self.attack_label.setText(f"used {game_vars.artefacts_dic[self.attack_row['artefact']]['name']} against")
        self.artefact_image_label.setPixmap(QtGui.QIcon(game_vars.artefacts_dic[self.attack_row['artefact']]['icon']).pixmap(40))
        self.victim_user_image.setPixmap(self.users_images_dic[self.attack_row['destination_user']])
