...
class project_users_widget(QtWidgets.QWidget):
    ...
    def refresh_tag_groups(self):
        ...
        for tag_group_row in all_tag_groups:
            ...
            if tag_group_row['id'] not in self.all_tag_groups:
                ...
                self.all_tag_groups[tag_group_row['id']]['widget'] = tag_group_widget(
                    tag_group_row, self.all_users_ids)
                self.tag_groups_container.addWidget(
                    self.all_tag_groups[tag_group_row['id']]['widget'])
            else:
                self.all_tag_groups[tag_group_row['id']
                                    ]['widget'].tag_group_row = tag_group_row
    ...
    def refresh_users_list(self):
        ...
        for user_row in all_users:
            ...
            self.all_users_ids[user_row['id']]['pixmap'] = gui_utils.mask_image(
                image.convert_str_data_to_image_bytes(user_row['profile_picture']), 'png', 30)
        ...
        for user_id in project.get_users_ids_list():
            ...
            if user_id not in self.users_ids.keys():
                widget = user_widget(
                    self.all_users_ids[user_id]['row'], self.all_users_ids[user_id]['pixmap'])
                widget.remove_user_signal.connect(
    ...
    def build_ui(self):
        ...
        self.info_label = QtWidgets.QLabel(
            "This list shows all the users who have logged in the project.")
        ...
        self.tag_groups_header_layout.addSpacerItem(QtWidgets.QSpacerItem(
            0, 0, QtWidgets.QSizePolicy.Policy.Expanding, QtWidgets.QSizePolicy.Policy.Fixed))

        self.create_tag_group_button = QtWidgets.QPushButton(
            'Create tag group')
        ...
        self.tags_info_label = QtWidgets.QLabel(
            "Manage here the creation and subscription of tag groups.")
...
class tag_group_widget(QtWidgets.QFrame):
    ...
    def build_ui(self):
        ...
        self.tag_group_image.setPixmap(
            QtGui.QIcon(ressources._tag_icon_).pixmap(23))
        ...
        self.delete_tag_group_button.setIcon(
        ...
        self.users_layout.addSpacerItem(QtWidgets.QSpacerItem(
            0, 0, QtWidgets.QSizePolicy.Policy.Expanding, QtWidgets.QSizePolicy.Policy.Fixed))
...
class user_widget(QtWidgets.QFrame):
    ...
    def connect_functions(self):
        self.ignore_user_button.clicked.connect(
            lambda: self.remove_user_signal.emit(self.user_row['id']))
