...
class championship_widget(QtWidgets.QWidget):
    ...
    def build_ui(self):
        ...
        self.user_header_layout.addSpacerItem(QtWidgets.QSpacerItem(
        ...
        self.deaths_item_label.setPixmap(QtGui.QIcon(
            ressources._skull_item_icon_).pixmap(25))
        self.items_layout.addWidget(self.deaths_item_label)

        self.comment_item_label = QtWidgets.QLabel()
        self.comment_item_label.setPixmap(QtGui.QIcon(
            ressources._green_item_icon_).pixmap(25))
        self.items_layout.addWidget(self.comment_item_label)

        self.worker_item_label = QtWidgets.QLabel()
        self.worker_item_label.setPixmap(
            QtGui.QIcon(ressources._red_item_icon_).pixmap(25))
        self.items_layout.addWidget(self.worker_item_label)

        self.xp_item_label = QtWidgets.QLabel()
        self.xp_item_label.setPixmap(QtGui.QIcon(
            ressources._yellow_item_icon_).pixmap(25))
        ...
        self.level_icon.setPixmap(QtGui.QIcon(
            ressources._level_icon_).pixmap(25))
        ...
        self.purse_icon.setPixmap(QtGui.QIcon(
            ressources._coin_icon_).pixmap(25))
        ...
        self.life_icon.setPixmap(QtGui.QIcon(
            ressources._heart_icon_).pixmap(25))
        ...
        self.tabs_widget.addTab(self.users_championship_widget, QtGui.QIcon(
            ressources._gold_icon_), 'Kingdom')

        self.inventory_widget = inventory_widget.inventory_widget()
        self.tabs_widget.addTab(self.inventory_widget, QtGui.QIcon(
            ressources._purse_icon_), 'Inventory')
        self.market_widget = market_widget.market_widget()
        self.tabs_widget.addTab(self.market_widget, QtGui.QIcon(
            ressources._market_icon_), 'Market')

        self.attack_history_widget = attack_history_widget.attack_history_widget()
        self.tabs_widget.addTab(self.attack_history_widget, QtGui.QIcon(
            ressources._attacks_history_icon_), 'Attacks History')
    ...
    def refresh_championship_participation(self, user_row):
        ...
        self.participation_checkbox.setChecked(
            user_row['championship_participation'])
        if not user_row['championship_participation']:
            self.participation_info_label.setText(
                "You don't participate to championship. You can't attack or be attacked.")
        else:
            self.participation_info_label.setText("")
        self.modify_participation = True
        ...
        if user_row['level'] != self.old_level:
            if user_row['level'] > self.old_level:
                gui_server.custom_popup(
                    f"You are now level {user_row['level']}", 'Congratulation', ressources._level_icon_)
            else:
                gui_server.custom_popup(
                    f"You are now level {user_row['level']}", 'You just lost a level, take care of your comments', ressources._level_icon_)
            self.old_level = user_row['level']
    ...
    def refresh_keeped_artefacts(self, user_row):
        ...
        for artefact in list(set(keeped_artefacts_dic.values())):
            if artefact not in self.keeped_artefacts_dic.keys():
                ...
                keeped_artefact_label.setPixmap(QtGui.QIcon(
                    game_vars.artefacts_dic[artefact]['icon']).pixmap(25))
    ...
    def modify_championship_participation(self):
        ...
        if current_participation:
            self.confirm_widget = confirm_widget.confirm_widget(
                f"If you turn off your participation to the game, you won't be able to participate again within {tools.convert_seconds_to_string_time_with_days(game_vars._default_ban_time_)}.\nDo you want to continue ?", parent=self)
            if self.confirm_widget.exec() != QtWidgets.QDialog.DialogCode.Accepted:
                return
        repository.modify_user_championship_participation(
            environment.get_user(), 1-current_participation)
    ...
    def remove_keeped_artefact(self, artefact):
        ...
        gui_server.custom_popup(
            f"Inventory", f"{game_vars.artefacts_dic[artefact]['name']} is expired !", game_vars.artefacts_dic[artefact]['icon'])
    ...
    def connect_functions(self):
        ...
        self.participation_checkbox.stateChanged.connect(
            self.modify_championship_participation)
    ...
    def items_check(self, user_row, user_rows):
        ...
        if user_row['id'] != xp_dic[sorted(list(xp_dic.keys()))[-1]]:
            ...
            if not self.is_first_xp and not (self.first_refresh):
                gui_server.custom_popup(
                    "You have earned so much experience !", 'Congratulation', ressources._yellow_item_icon_)
            self.is_first_xp = 1
        ...
        if user_row['id'] != deaths_dic[sorted(list(deaths_dic.keys()))[-1]]:
            ...
            if not self.is_first_deaths and not (self.first_refresh):
                gui_server.custom_popup("You are the one who die the most !",
                                        'Try to comment your work more often', ressources._skull_item_icon_)
        ...
        if user_row['id'] != comments_dic[sorted(list(comments_dic.keys()))[-1]]:
            ...
            if not self.is_first_comments_count and not (self.first_refresh):
                gui_server.custom_popup(
                    "You are really good at commenting !", 'Congratulation', ressources._green_item_icon_)
        ...
        if user_row['id'] != work_time_dic[sorted(list(work_time_dic.keys()))[-1]]:
            ...
            if not self.is_first_work_time and not (self.first_refresh):
                gui_server.custom_popup(
                    "You worked so much time !", 'Congratulation', ressources._red_item_icon_)
