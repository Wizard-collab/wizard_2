...
class video_player_widget(QtWidgets.QFrame):
    ...
    def show_preferences(self):
        self.preferences_dialog = preferences_widget(
            self.fps, self.resolution, self)
    ...
    def add_video(self, video_file, project_video_id=None):
        ...
        self.videos_dic[video_id]['original_file'] = path_utils.abspath(
            video_file)
        self.videos_dic[video_id]['name'] = path_utils.basename(video_file)
        self.videos_dic[video_id]['frames_count'] = ffmpeg_utils.get_frames_count(
            video_file)
        self.videos_dic[video_id]['inpoint'] = 0
        self.videos_dic[video_id]['outpoint'] = self.videos_dic[video_id]['frames_count']
        self.videos_dic[video_id]['thumbnail'] = ffmpeg_utils.extract_first_frame(
            video_file, self.temp_dir)
    ...
    def build_ui(self):
        ...
        self.menu_layout.addSpacerItem(QtWidgets.QSpacerItem(
            0, 0, QtWidgets.QSizePolicy.Policy.Expanding, QtWidgets.QSizePolicy.Policy.Fixed))

        self.player_action = gui_utils.add_menu_to_menu_bar(
            self.menu_bar, title='Player')
        self.show_preferences_action = self.player_action.addAction(
            QtGui.QIcon(ressources._settings_icon_), "Preferences")
        self.playlist_action = gui_utils.add_menu_to_menu_bar(
            self.menu_bar, title='Playlist')
        self.save_playlist_action = self.playlist_action.addAction(
            QtGui.QIcon(ressources._tool_save_), "Save playlist")
        self.save_as_new_playlist_action = self.playlist_action.addAction(
            QtGui.QIcon(ressources._tool_save_), "Save as new playlist")
        self.clear_playlist_action = self.playlist_action.addAction(
            QtGui.QIcon(ressources._tool_add_), "New playlist")
        self.export_video_file_action = self.playlist_action.addAction(
            QtGui.QIcon(ressources._tool_batch_publish_), "Export video")
        ...
        self.playlist_icon_label.setPixmap(
            QtGui.QIcon(ressources._playlist_icon_).pixmap(22))
        ...
        self.playlist_layout.addSpacerItem(QtWidgets.QSpacerItem(
            0, 0, QtWidgets.QSizePolicy.Policy.Expanding, QtWidgets.QSizePolicy.Policy.Fixed))
    ...
    def connect_functions(self):
        ...
        self.video_player.on_frame_range_modified.connect(
            self.update_frame_range)
        self.video_player.on_play_toggle.connect(
            self.timeline_widget.set_play_pause)
        self.timeline_widget.on_seek.connect(self.video_player.seek_frame)
        self.timeline_widget.on_play_pause.connect(
            self.video_player.play_pause_toggle)
        self.timeline_widget.on_prev_frame.connect(
            self.video_player.previous_frame)
        self.timeline_widget.on_next_frame.connect(
            self.video_player.next_frame)
        self.timeline_widget.on_loop_toggle.connect(
            self.video_player.toggle_loop)
        self.timeline_widget.on_bounds_change.connect(
            self.video_player.set_bounds_range)
        self.timeline_widget.on_end_requested.connect(
            self.video_player.seek_end)
        self.timeline_widget.on_beginning_requested.connect(
            self.video_player.seek_beginning)
        self.timeline_widget.on_order_changed.connect(self.order_changed)
        self.timeline_widget.on_video_in_out_modified.connect(
            self.in_out_modified)
        ...
        self.timeline_widget.current_video_row.connect(
        ...
        self.save_as_new_playlist_action.triggered.connect(
    ...
    def order_changed(self, new_order):
        self.videos_dic = dict(
            sorted(self.videos_dic.items(), key=lambda x: new_order.index(x[0])))
        self.update_concat()
        self.set_modified(True)

    def in_out_modified(self, modification_dic):
        self.videos_dic[modification_dic['id']
                        ]['inpoint'] = modification_dic['inpoint']
        self.videos_dic[modification_dic['id']
                        ]['outpoint'] = modification_dic['outpoint']
        self.update_concat()
        self.set_modified(True)
    ...
    def set_modified(self, is_modified):
        ...
        if self.modified:
            self.current_playlist_label.setText(
    ...
    def save_playlist(self):
        if self.current_playlist is not None:
            assets.save_playlist(self.current_playlist, self.videos_dic,
                                 thumbnail_temp_path=self.get_first_thumbnail())
            self.set_modified(False)
    ...
    def save_as_new_playlist(self):
        self.create_playlist_widget = create_playlist_widget.create_playlist_widget(
            data=json.dumps(self.videos_dic), thumbnail_temp_path=self.get_first_thumbnail())
    ...
    def export_video_file(self):
        file_name, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, "Save video file", "", "Mp4 Files (*.mp4)")
        if file_name:
            concat_file = ffmpeg_utils.get_concat_video_file(
                self.temp_dir, self.player_id)
...
class preferences_widget(QtWidgets.QDialog):
    ...
    def build_ui(self):
        ...
        self.fps_spinbox.setButtonSymbols(
        ...
        self.settings_layout.addRow(QtWidgets.QLabel(
            'Resolution'), self.resolutions_layout)
