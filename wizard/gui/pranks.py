...
class pranks():
    ...
    def execute_attack(self, signal_dic):
        if 'mouse_attack' in signal_dic['attack_type']:
            ...
            self.pranks_dic[time_id].set_duration(game_vars.artefacts_dic[signal_dic['attack_type']]['duration'])
            ...
            logger.info(f"Your screen is flipped by {signal_dic['from_user']} for 10s")
            gui_server.custom_popup(f"Mouahahah", f"Your screen is flipped by {signal_dic['from_user']} for 10 seconds, try to turn your head !", game_vars.artefacts_dic[signal_dic['attack_type']]['icon'])
            time_id = time.time()
            ...
            logger.info(f"{signal_dic['from_user']} just modified the speed of your mouse, good luck !")
            gui_server.custom_popup(f"Mouahahah", f"{signal_dic['from_user']} just modified the speed of your mouse, good luck !", game_vars.artefacts_dic[signal_dic['attack_type']]['icon'])
            mouse_speed_attack()
