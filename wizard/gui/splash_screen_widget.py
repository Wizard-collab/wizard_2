...
class splash_screen_widget(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        self.setWindowFlags(QtCore.Qt.WindowType.CustomizeWindowHint | QtCore.Qt.WindowType.WindowStaysOnTopHint |
    ...
    def build_ui(self):
        ...
        self.main_frame.setStyleSheet(
            "#dark_round_frame{background: qlineargradient(spread:pad, x1:1 y1:0, x2:1 y2:1, stop:0 #141418, stop:1 #1d1d23);border-top-left-radius:8px;border-top-right-radius:8px;}")
        ...
        self.header_widget.setStyleSheet(
            '#splash_screen_header{border-top-left-radius:8px;border-top-right-radius:8px;}')
        ...
        self.wizard_icon.setPixmap(QtGui.QIcon(
            ressources._wizard_icon_).pixmap(40))
        ...
        self.recent_scenes_list.setStyleSheet(
            '#tree_as_list_widget{background-color:transparent}')
        ...
        self.recent_scenes_list.setSelectionMode(
            QtWidgets.QAbstractItemView.SelectionMode.SingleSelection)
        self.recent_scenes_list.setSizePolicy(
            QtWidgets.QSizePolicy.Policy.Expanding, QtWidgets.QSizePolicy.Policy.Fixed)
        ...
        self.updates_scrollArea_widget.setLayout(
            self.updates_scrollArea_layout)

        self.updates_scrollArea.setHorizontalScrollBarPolicy(
        ...
        self.updates_scrollArea_layout.addSpacerItem(QtWidgets.QSpacerItem(
            0, 0, QtWidgets.QSizePolicy.Policy.Expanding, QtWidgets.QSizePolicy.Policy.Expanding))
    ...
    def fill_ui(self):
        self.show_at_startup_checkbox.setChecked(
            user.user().get_show_splash_screen())
        self.version_dic = application.get_version()
        self.version_label.setText(
            f"{self.version_dic['MAJOR']}.{self.version_dic['MINOR']}.{self.version_dic['PATCH']} - build {self.version_dic['builds']}")
        self.fill_whats_new()
    ...
    def refresh_recent_scenes(self):
        ...
        for work_env_tuple in recent_work_env_ids:
            if not project.check_work_env_existence(work_env_tuple[0]):
                continue
            item = recent_scene_item(
                work_env_tuple, self.recent_scenes_list.invisibleRootItem())
        self.recent_scenes_list.setFixedHeight(
            self.recent_scenes_list.invisibleRootItem().childCount()*31)
    ...
    def connect_functions(self):
        self.show_at_startup_checkbox.stateChanged.connect(
            self.toggle_show_at_startup)
        self.recent_scenes_list.itemDoubleClicked.connect(
            self.launch_recent_work_env)
...
class update_area(QtWidgets.QFrame):
    def __init__(self, update_dic, update, current_version_key, parent=None):
        ...
        self.setStyleSheet(
            "#round_frame{background-color:rgba(255,255,255,5)}")
        self.update = update
    ...
    def build_ui(self):
        ...
        self.main_layout.addSpacerItem(QtWidgets.QSpacerItem(
            0, 12, QtWidgets.QSizePolicy.Policy.Expanding, QtWidgets.QSizePolicy.Policy.Fixed))
    ...
    def fill_ui(self):
        ...
        self.date_label.setText(
            f"{day} ( {tools.time_ago_from_timestamp(self.update_dic['time'])} )")
        if self.update != 'last':
...
class update_frame(QtWidgets.QFrame):
    def __init__(self, update_type, update, parent=None):
        ...
        self.setObjectName('transparent_widget')
        self.setStyleSheet(
            '#transparent_widget{border-top:1px solid rgba(255,255,255,6)}')
...
class recent_scene_item(QtWidgets.QTreeWidgetItem):
    ...
    def fill_ui(self):
        ...
        icon = gui_utils.QIcon_from_svg(ressources._stage_icons_dic_[
                                        stage_row['name']], 'gray')
        software_icon = gui_utils.QIcon_from_svg(ressources._softwares_icons_dic_[
                                                 work_env_row['name']], 'gray')
