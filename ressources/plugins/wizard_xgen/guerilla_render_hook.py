...
def after_reference(stage_name,
    ...
    if referenced_stage_name == 'grooming':
        ...
        GRP = wizard_tools.get_node_from_name('GROOMING')

        for file in os.listdir(referenced_files_dir):
            namespace = namespace+'_'+file.split('.')[0].split('_')[-1]
            if 'xgen_cache' in file:
                full_file = os.path.join(referenced_files_dir, file)
                from guerilla_render_wizard import wizard_reference
                logger.info(wizard_tools.get_all_nodes())
                if namespace in wizard_tools.get_all_nodes():
                    wizard_reference.update_ref(namespace, [full_file], GRP)
                else:
                    wizard_reference.create_ref(namespace, [full_file], GRP)
