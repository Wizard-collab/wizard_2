...
class widget(QtWidgets.QWidget):
    ...
    def build_ui(self):
        ...
        self.info_label_1 = QtWidgets.QLabel("""Select the stage you want to propagate below""")
    ...
    for stage_id in stages_ids_list:
        ...
        for export_id in exports_ids:
            ...
            if not export_version_id:
                continue
            destination_references_rows = project.get_export_version_destinations(export_version_id)
            for destination_reference_row in destination_references_rows:
                work_env_ids_list.append(destination_reference_row['work_env_id'])
            destination_grouped_references_rows = project.get_grouped_export_version_destination(export_version_id)
...
def export_work_envs(work_env_ids_list, comment=''):
    ...
    for work_env_id in work_env_ids_list:
        ...
        if not work_env_row:
            continue
        last_version_id = project.get_last_work_version(work_env_row['id'], 'id')[0]
