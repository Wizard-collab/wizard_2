...
name = "Create videos"
icon = ressources._tool_video_
description = """Publish a video for each selected stage
"""
...
class widget(QtWidgets.QWidget):
    ...
    def build_ui(self):
        ...
        self.info_label_1 = QtWidgets.QLabel("""Select the stages for creating videos below and
then click on execute""")
    ...
    def execute(self):
        ...
        command = "from ressources.batcher_scripts import create_videos\n"
        command += f"create_videos.main({stage_ids}, {refresh_assets}, comment='''{comment}''')"
...
def main(stages_ids_list, refresh_assets, comment=''):
    ...
    for stage_id in stages_ids_list:
        ...
        namespaces_list = []
        references_dic = assets.get_references_files(
            default_work_env_row['id'])
        for reference_type in references_dic.keys():
            if reference_type not in ['camrig', 'camera']:
                continue
        ...
        print(
            f"wizard_task_name:Creating video for {asset_row['name']}/{stage_row['name']}")
