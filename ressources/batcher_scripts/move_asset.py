...
class widget(QtWidgets.QWidget):
    ...
    def build_ui(self):
        ...
        self.original_asset_label = QtWidgets.QLabel(
            "Select here the asset to move")
        ...
        self.original_asset_layout.addWidget(
            self.original_asset_tree_info_label)

        self.destination_category_layout = QtWidgets.QVBoxLayout()
        self.trees_layout.addLayout(self.destination_category_layout)
        self.destination_category_label = QtWidgets.QLabel(
            "Select here destination category")
        self.destination_category_layout.addWidget(
            self.destination_category_label)
        ...
        self.destination_category_layout.addWidget(
            self.destination_category_tree)
        ...
        self.destination_category_layout.addWidget(
            self.destination_category_tree_info_label)
    ...
    def fill_original_asset_tree(self):
        ...
        for domain_row in all_domains:
            ...
            domain_item = QtWidgets.QTreeWidgetItem(
                self.original_asset_tree.invisibleRootItem())
            ...
            for category_row in categories:
                ...
                for asset_row in assets:
                    ...
                    self.original_asset_tree.setItemWidget(
                        asset_item, 0, check_box)
    ...
    def fill_destination_category_tree(self):
        ...
        for domain_row in all_domains:
            categories = project.get_domain_childs(domain_row['id'])
            domain_item = QtWidgets.QTreeWidgetItem(
                self.destination_category_tree.invisibleRootItem())
            domain_item.setText(0, domain_row['name'])
            for category_row in categories:
                category_item = QtWidgets.QTreeWidgetItem(domain_item)
                check_box = QtWidgets.QCheckBox(category_row['name'])
                check_box.stateChanged.connect(self.refresh)
                self.destination_category_tree.setItemWidget(
                    category_item, 0, check_box)
                self.destination_category_dic[category_row['id']] = dict()
                self.destination_category_dic[category_row['id']
                                              ]['row'] = category_row
                self.destination_category_dic[category_row['id']
                                              ]['item'] = category_item
                self.destination_category_dic[category_row['id']
    ...
    def refresh(self):
        ...
        if len(assets_ids) > 1:
            self.original_asset_tree_info_label.setText(
                "Please select only one asset to move")
            self.original_asset_tree_info_label.setStyleSheet("color:#f79360")
        if len(assets_ids) == 1:
            self.original_asset_tree_info_label.setText("")
...
def main(original_asset_id, destination_category_id, new_name):
    ...
    destination_asset_id = assets.create_asset(
        new_name, destination_category_id)
    ...
    for stage_row in stages_list:
        ...
        # CREATING STAGES
        destination_stage_id = assets.create_stage(
            stage_row['name'], destination_asset_id)
        original_variants = project.get_stage_childs(stage_row['id'])
        for variant_row in original_variants:

            # CREATING VARIANTS
            if variant_row['name'] != 'main':
                assets.create_variant(
                    variant_row['name'], destination_stage_id)
            destination_variant_id = project.get_variant_by_name(
                destination_stage_id, variant_row['name'], column='id')
            original_work_envs = project.get_variant_work_envs_childs(
                variant_row['id'])

            # CREATING WORK ENVS
            for work_env_row in original_work_envs:
                destination_work_env_id = assets.create_work_env(
                    work_env_row['software_id'], destination_variant_id)

                # COPY WORK VERSIONS
                work_versions = project.get_work_versions(work_env_row['id'])
                new_work_versions = []
                for work_version_row in work_versions:
                    new_version_id = assets.duplicate_version(
                        work_version_row['id'], destination_work_env_id)
