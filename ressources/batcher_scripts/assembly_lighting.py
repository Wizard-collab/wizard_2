...
class widget(QtWidgets.QWidget):
    ...
    def build_ui(self):
        ...
        self.lighting_stages_label = QtWidgets.QLabel(
            "Select here the lighting stages")
        ...
        self.lighting_stages_layout.addWidget(
            self.lighting_stages_project_tree)
        ...
        self.lighting_stages_label = QtWidgets.QLabel(
            "Select here additionnal references to add to your assembly")
        self.additionnal_references_stages_layout.addWidget(
            self.lighting_stages_label)

        self.additionnal_references_stages_project_tree = QtWidgets.QTreeWidget()
        self.additionnal_references_stages_project_tree.setObjectName(
            'tree_widget')
        self.additionnal_references_stages_project_tree.setHeaderHidden(True)
        self.additionnal_references_stages_project_tree.setAlternatingRowColors(
            True)
        self.additionnal_references_stages_layout.addWidget(
            self.additionnal_references_stages_project_tree)
    ...
    def fill_lighting_stages_project_tree(self):
        ...
        for domain_row in all_domains:
            ...
            domain_item = QtWidgets.QTreeWidgetItem(
                self.lighting_stages_project_tree.invisibleRootItem())
            ...
            for category_row in categories:
                ...
                for asset_row in assets:
                    ...
                    for stage_row in stages:
                        ...
                        self.lighting_stages_project_tree.setItemWidget(
    ...
    def fill_additionnal_references_project_tree(self):
        ...
        for domain_row in all_domains:
            categories = project.get_domain_childs(domain_row['id'])
            domain_item = QtWidgets.QTreeWidgetItem(
                self.additionnal_references_stages_project_tree.invisibleRootItem())
            domain_item.setText(0, domain_row['name'])
            for category_row in categories:
                assets = project.get_category_childs(category_row['id'])
                category_item = QtWidgets.QTreeWidgetItem(domain_item)
                ...
                for asset_row in assets:
                    ...
                    for stage_row in stages:
                        ...
                        check_box = QtWidgets.QCheckBox(stage_row['name'])
                        check_box.stateChanged.connect(self.refresh)
                        self.additionnal_references_stages_project_tree.setItemWidget(
                            stage_item, 0, check_box)
                        self.additionnal_references_stages_dic[stage_row['id']] = dict(
                        )
                        self.additionnal_references_stages_dic[stage_row['id']
                                                               ]['row'] = stage_row
                        self.additionnal_references_stages_dic[stage_row['id']
                                                               ]['item'] = stage_item
                        self.additionnal_references_stages_dic[stage_row['id']
                                                               ]['check_box'] = check_box
        groups_item = QtWidgets.QTreeWidgetItem(
            self.additionnal_references_stages_project_tree.invisibleRootItem())
        ...
        for group_row in groups:
            ...
            check_box.stateChanged.connect(self.refresh)
            self.additionnal_references_stages_project_tree.setItemWidget(
                group_item, 0, check_box)
            self.additionnal_references_groups_dic[group_row['id']] = dict()
            self.additionnal_references_groups_dic[group_row['id']
                                                   ]['row'] = group_row
            self.additionnal_references_groups_dic[group_row['id']
                                                   ]['item'] = group_item
            self.additionnal_references_groups_dic[group_row['id']
...
def main(stages_ids_list, additionnal_references_stages_ids_list, additionnal_references_groups_ids, comment=''):
    ...
    for stage_id in stages_ids_list:
        ...
        default_variant_row = project.get_variant_data(
            stage_row['default_variant_id'])
        if not default_variant_row:
            logger.info(
                f"Default variant of {stage_row['name']} not found, skipping.")
            continue
        default_work_env_row = project.get_work_env_data(
            default_variant_row['default_work_env_id'])
        if not default_work_env_row:
            logger.info(
                f"Default work environment of {stage_row['name']}/{default_variant_row['name']} not found, skipping.")
            continue
        last_version_id = project.get_last_work_version(
            default_work_env_row['id'], 'id')[0]
        if not last_version_id:
            logger.info(
                f"No work version found for {stage_row['name']}/{default_variant_row['name']}/{default_work_env_row['name']}, skipping.")
            continue

        # GETTING EXISTING REFERENCES
        existing_references = project.get_references(
            default_work_env_row['id'], 'export_id')
        ...
        for other_stage_row in project.get_asset_childs(asset_row['id']):
            ...
            if other_stage_row['name'] == 'compositing':
                continue
            export_rows = project.get_stage_export_childs(
                other_stage_row['id'])
        ...
        for additionnal_reference_stage_id in additionnal_references_stages_ids_list:
            ...
            for export_row in export_rows:
                ...
                if export_version_id:
                    assets.create_reference(
                        default_work_env_row['id'], export_version_id)
        ...
        for additionnal_reference_group_id in additionnal_references_groups_ids:
            assets.create_referenced_group(
                default_work_env_row['id'], additionnal_reference_group_id)
